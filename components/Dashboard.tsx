...
import { DashboardIcon, FlagIcon, SearchIcon, StarIcon, CalendarIcon, SparklesIcon, TrendingIcon, CheckCircleIcon, UserIcon, ArrowUpCircleIcon, KanbanIcon } from './icons/UIIcons';
...
const QuickAction: React.FC<{ icon: React.ReactNode; label: string; onClick: () => void; color: string }> = ({ icon, label, onClick, color }) => (
    <button onClick={onClick} className={`flex flex-col items-center justify-center p-4 rounded-xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-md transition-all group`}>
        <div className={`p-3 rounded-full mb-3 text-white transition-transform group-hover:scale-110 ${color}`}>
            {icon}
        </div>
    ...
    <div onClick={onClick} className="flex items-center p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-100 dark:border-gray-700 shadow-sm cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
        ...
        <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center mt-1">
                ...
                <span className="text-gray-400 mr-2 scale-75">{platformFilterIcons[event.platform]}</span>
                <span className={`text-xs px-2 py-0.5 rounded-full ${event.status === 'Scheduled' ? 'bg-blue-100 text-blue-700' : event.status === 'Published' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                    {event.status}
                </span>
            </div>
        </div>
...
export const Dashboard: React.FC = () => {
  ...
  const isAgencyOrElite = ['Agency', 'Elite'].includes(user?.plan || '');

  // Highlighting and filtering from context
  useEffect(() => {
    if (dashboardNavState) {
        ...
        if (dashboardNavState.highlightId) {
            ...
            setViewMode('Inbox'); // Auto-switch to inbox if navigating to a message
        }
  ...
  const handleBulkArchive = () => {
    selectedMessageIds.forEach(id => updateMessage(id, { isArchived: true }));
    setSelectedMessageIds(new Set());
  };

  const handleBulkDelete = () => {
    if (window.confirm(`Are you sure you want to delete ${selectedMessageIds.size} selected messages? This action cannot be undone.`)) {
  ...
  const handleAutoRespondToggle = () => {
    setSettings(prev => ({ ...prev, autoRespond: !prev.autoRespond }));
  };
  
  const handleToggleFlag = (id: string) => {
      const msg = messages.find(m => m.id === id);
      if (msg) updateMessage(id, { isFlagged: !msg.isFlagged });
  };

  const handleToggleFavorite = (id: string) => {
      const msg = messages.find(m => m.id === id);
      if (msg) updateMessage(id, { isFavorite: !msg.isFavorite });
  };
      ...
      <div className="space-y-8 animate-fade-in">
          ...
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
             ...
             <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  ...
                  <QuickAction label="Generate Strategy" icon={<TrendingIcon className="w-6 h-6" />} color="bg-gradient-to-br from-blue-500 to-cyan-500" onClick={() => setActivePage('strategy')} />
                  ...
                  <QuickAction label="Check Analytics" icon={<CheckCircleIcon className="w-6 h-6" />} color="bg-gradient-to-br from-emerald-400 to-green-600" onClick={() => setActivePage('analytics')} />
              </div>
          </div>

          {/* Audience Stats */}
          <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Audience Stats</h3>
  ...
  const renderInbox = () => (
    <div className="space-y-6 animate-fade-in">
                ...
                <button onClick={categorizeAllMessages} className="text-sm font-medium text-primary-600 hover:underline">Categorize Inbox</button>
                 <div className="flex items-center gap-2 p-1.5 bg-gray-200 dark:bg-gray-700 rounded-full">
                    ...
                    <button
                        ...
                        aria-label={`Toggle auto-respond for ${accountName}`}
        ...
        <div className="flex border-b border-gray-200 dark:border-gray-700">
            {(['All', 'Lead', 'Support', 'Opportunity', 'General'] as const).map(cat => (
                 <button key={cat} onClick={() => handleFilterChange('category', cat)} className={`px-4 py-2 text-sm font-medium ${filters.category === cat ? 'border-b-2 border-primary-500 text-primary-600' : 'text-gray-500 hover:text-gray-700'}`}>
                     {cat}
                 </button>
            ))}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center space-x-1 bg-gray-200 dark:bg-gray-700 p-1 rounded-full">
                <FilterButton isActive={filters.platform === 'All'} onClick={() => handleFilterChange('platform','All')} label="All">
                    <DashboardIcon />
                </FilterButton>
                {(Object.keys(platformFilterIcons) as Platform[]).map(platform => (
                    <FilterButton key={platform} isActive={filters.platform === platform} onClick={() => handleFilterChange('platform',platform)} label={platform}>
                        {platformFilterIcons[platform]}
                    </FilterButton>
                ))}
            </div>
             <div className="flex items-center space-x-1 bg-gray-200 dark:bg-gray-700 p-1 rounded-full">
                <FilterButton isActive={filters.messageType === 'All'} onClick={() => handleFilterChange('messageType', 'All')} label="All Types" />
                <FilterButton isActive={filters.messageType === 'DM'} onClick={() => handleFilterChange('messageType', 'DM')} label="DMs" />
                <FilterButton isActive={filters.messageType === 'Comment'} onClick={() => handleFilterChange('messageType', 'Comment')} label="Comments" />
            </div>
            <div className="flex items-center space-x-1 bg-gray-200 dark:bg-gray-700 p-1 rounded-full">
                <FilterButton isActive={filters.status === 'All'} onClick={() => handleFilterChange('status', 'All')} label="All Status" />
                <FilterButton isActive={filters.status === 'Favorite'} onClick={() => handleFilterChange('status', 'Favorite')} label="Favorites">
                    <StarIcon className="w-4 h-4" />
                </FilterButton>
                <FilterButton isActive={filters.status === 'Flagged'} onClick={() => handleFilterChange('status', 'Flagged')} label="Flagged">
      ...
      {isLoading ? (
        <div className="text-center py-16">
            ...
            <p className="mt-4 text-lg font-semibold text-gray-700 dark:text-gray-300">Loading messages...</p>
        </div>
      ...
      {selectedMessageIds.size > 0 && (
          <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-40 w-auto bg-white dark:bg-gray-800 shadow-2xl rounded-full p-3 flex items-center gap-4">
              <span className="font-semibold px-2">{selectedMessageIds.size} selected</span>
              <button className="px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-full hover:bg-primary-700">Approve All</button>
              ...
              <button onClick={handleBulkDelete} className="px-4 py-2 text-sm font-medium text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-900/50 rounded-full hover:bg-red-200 dark:hover:bg-red-900">Delete All</button>
          </div>
      )}
    </div>
  );
         ...
         <div className="flex justify-end mb-2">
            <div className="bg-gray-100 dark:bg-gray-700 p-1 rounded-lg inline-flex">
                <button 
                    ...
                    className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'Overview' ? 'bg-white dark:bg-gray-600 text-primary-600 dark:text-white shadow' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900'}`}
                >
                    Command Center
                </button>
                <button 
                    onClick={() => setViewMode('Inbox')}
                    className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${viewMode === 'Inbox' ? 'bg-white dark:bg-gray-600 text-primary-600 dark:text-white shadow' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900'}`}
                >
                    Full Inbox
                </button>
            </div>
