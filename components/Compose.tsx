      ...
      <div className="space-y-6">
        ...
        {(
          <div className="flex flex-wrap items-center gap-3 p-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 mb-6">
            ...
            <button
              onClick={async () => {
                // If there are media items, use the first one
                if (composeState.mediaItems.length > 0) {
                  const item = composeState.mediaItems[0];
                  if (!item.captionText.trim()) {
                    showToast('Please add a caption first.', 'error');
                    return;
                  }
                  const platformsToPost = (Object.keys(item.selectedPlatforms || {}) as Platform[]).filter(p => item.selectedPlatforms?.[p]);
                  if (platformsToPost.length === 0) {
                    showToast('Please select at least one platform.', 'error');
                    return;
                  }
                  await handleSaveToWorkflowMedia(0, 'Draft');
                } else {
                  // Text-only post
                  if (!composeState.captionText.trim()) {
                    showToast('Please add a caption first.', 'error');
                    return;
                  }
                  ...
                  await handleSaveToWorkflow('Draft');
                }
              }}
              disabled={isSaving || (!composeState.captionText.trim() && composeState.mediaItems.length === 0)}
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-md hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors disabled:opacity-50"
            >
            ...
            <button
              onClick={async () => {
                ...
                if (composeState.mediaItems.length > 0) {
                  ...
                  await handleSaveToWorkflowMedia(0, 'Approved');
                } else {
                  // Text-only post
                  if (!composeState.captionText.trim()) {
                    showToast('Please add a caption first.', 'error');
                  ...
                  // Create a temporary media item for Approved workflow
                  const tempItem: MediaItemState = {
                    id: Date.now().toString(),
                    previewUrl: '',
                    data: '',
                    mimeType: '',
                    type: 'image',
                    results: [],
                    captionText: composeState.captionText,
                    postGoal: composeState.postGoal,
                    postTone: composeState.postTone,
                    selectedPlatforms: selectedPlatforms,
                  };
                  setComposeState(prev => ({ ...prev, mediaItems: [tempItem] }));
                  setTimeout(() => {
                    handleSaveToWorkflowMedia(0, 'Approved');
                  }, 100);
                }
              }}
              disabled={isSaving || (!composeState.captionText.trim() && composeState.mediaItems.length === 0)}
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-900/30 border border-green-200 dark:border-green-800 rounded-md hover:bg-green-100 dark:hover:bg-green-900/50 transition-colors disabled:opacity-50"
            >
            ...
            <button
              onClick={async () => {
                ...
                if (composeState.mediaItems.length > 0) {
                  ...
                  if (!item.scheduledDate) {
                    showToast('Please set a schedule date first.', 'error');
                    return;
                  }
                  await handleScheduleMedia(0);
                } else {
                  // Text-only post
                  if (!composeState.captionText.trim()) {
                    showToast('Please add a caption first.', 'error');
                  ...
                  await handleSchedule(scheduleDate);
                }
              }}
              disabled={isSaving || (!composeState.captionText.trim() && composeState.mediaItems.length === 0) || (!scheduleDate && composeState.mediaItems.length === 0) || (composeState.mediaItems.length > 0 && !composeState.mediaItems[0].scheduledDate)}
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30 border border-purple-200 dark:border-purple-800 rounded-md hover:bg-purple-100 dark:hover:bg-purple-900/50 transition-colors disabled:opacity-50"
            >
              <CalendarIcon className="w-4 h-4" />
              Schedule
            </button>
            <button
              onClick={async () => {
                // If there are media items, use the first one
                if (composeState.mediaItems.length > 0) {
                  await handlePublishMedia(0);
                } else {
                  await handlePublish();
                }
              }}
              disabled={isSaving || (!composeState.captionText.trim() && composeState.mediaItems.length === 0)}
              className="flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-green-600 hover:bg-green-700 rounded-md transition-colors disabled:opacity-50"
            >
