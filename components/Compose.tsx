...
// 🔐 Ensure we have a logged-in user + fresh token before calling AI
...
const CaptionGenerator: React.FC = () => {
  ...
  // ✅ Backend media URL (Firebase Storage download URL)
  ...
  // Pre-fill from composeContext (calendar planning etc.)
  useEffect(() => {
  ...
  // ----------------------------------------
  // Upload media file → Firebase Storage
  // ----------------------------------------
  const uploadMediaAndGetUrl = async (file: File) => {
    if (!user) return null;

    const timestamp = Date.now();
    const ext = file.name.split('.').pop() || 'bin';

    const storagePath = `users/${user.id}/uploads/${timestamp}.${ext}`;
    const storageRef = storageApi.ref(storage, storagePath);

    const uploadTask = await storageApi.uploadBytes(storageRef, file, {
      contentType: file.type,
    });

    return await storageApi.getDownloadURL(uploadTask.ref);
  };
  ...
  const handleSchedule = async (date: string) => {
    ...
    setIsSaving(true);
    try {
      // Prefer the uploaded storage URL; fall back to previewUrl if it is already remote
      let mediaUrl = uploadedMediaUrl;
      if (!mediaUrl && composeState.media?.previewUrl?.startsWith('http')) {
        mediaUrl = composeState.media.previewUrl;
      }
  ...
  // ----------------------------------------
  // AI: Generate captions from mediaUrl
  // ----------------------------------------
  const callGenerateCaptions = async (mediaUrl: string | null) => {
    if (!canGenerate) {
      showToast(
    ...
    try {
      ...
      if (
        ...
        setError(
          'Failed to generate captions. Please verify your Vercel API + auth configuration.'
        );
      }
    } finally {
      setIsLoading(false);
  ...
  const handleFileChange = async (
    ...
    setComposeState((prev) => ({
      ...prev,
      media: {
        data: '', // not used anymore, but keeps shape
        mimeType: file.type,
        previewUrl: localUrl,
        type: fileType,
      },
    ...
    try {
      ...
      const storageUrl = await uploadMediaAndGetUrl(file);
      if (!storageUrl) {
        throw new Error('Failed to upload media.');
      }

      setUploadedMediaUrl(storageUrl);

      // Now call AI captions with mediaUrl
      await callGenerateCaptions(storageUrl);
    } catch (err: any) {
      console.error(err);
      showToast(
        err?.message || 'Failed to process media and generate captions.',
        'error'
      );
      setError('Failed to generate captions from this media.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleRegenerate = () => {
    if (!uploadedMediaUrl) {
      showToast(
        'No uploaded media found. Please upload an image or video again.',
        'error'
      );
      return;
    }
    callGenerateCaptions(uploadedMediaUrl);
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-white dark:bg-gray-800 rounded-xl p-6 w-full max-w-md">
            ...
            <div className="grid grid-cols-3 gap-2 mb-6">
              {(
                ['X', 'LinkedIn', 'Instagram', 'TikTok', 'Threads', 'Facebook'] as Platform[]
              ).map((p) => (
                <button
                  key={p}
...
export const Compose: React.FC = () => {
    ...
    if (user) {
      setUser({
        ...
        monthlyImageGenerationsUsed:
          user.monthlyImageGenerationsUsed + 1,
      });
    }
  };
    ...
    if (user) {
      setUser({
        ...
        monthlyVideoGenerationsUsed:
          user.monthlyVideoGenerationsUsed + 1,
      });
    }
  };
  ...
  const renderTabContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'video':
        // Video backend not implemented: keep same gating logic
