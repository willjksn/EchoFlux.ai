...
- **Landing store preview:** In **My Page**, turn on **Show store items on landing** (`publicTreatsOnLanding` / Fan Hub store settings). The landing “Store” block lets visitors preview products marked for the landing (`showOnLandingPage`), but purchases happen from the member store after they join or sign in.
