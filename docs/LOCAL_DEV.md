...
- **Guest treat shop on the landing:** In **My Page**, turn on **public treats on landing** (`publicTreatsOnLanding` / Fan Hub store settings). Then the “Shop treats” block calls **guest checkout** (`guestProduct`) for visitors who are **not** subscribed members on that page—no sign-up required to open the modal. Products must be marked to show on the landing (`showOnLandingPage`); the CTA still appears while the list loads (or if empty) so the flow is not replaced by “sign up first.”
