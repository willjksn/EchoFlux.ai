...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    try {
      // ----------------------------------------------------
      // 1) Use cached weekly trends (NO live Tavily for users)
      // ----------------------------------------------------
      let webContextText = "";
      try {
        webContextText = await getLatestTrends();
      } catch (err) {
        console.error("[findTrendsByNiche] Failed to load cached weekly trends:", err);
